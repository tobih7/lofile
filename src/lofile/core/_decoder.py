...
from lofile.core.shared import BaseClass, DESCRIPTION_MAX_LENGTH, DataType, DecodeError, LogLvl, NONETYPE, TAG_MAX_LENGTH, binary_to_attrib, binary_to_timestamp, decompress, decrypt
...
class Decoder(BaseClass):
    ...
    def read(self, file: BinaryIO, password: Optional[bytes] = None) -> Any:
        ...
        if password is not None: # if it is None, file may be unencrypted, but maybe the password was already set via attribute
        ...
        if file.read(1) == b"\x03":
            ...
            if not self.__isencrypted:
                self.log("AES initialization vector is present, but file seems to be unencrypted", LogLvl.WARN)
        else:
            file.seek(file.tell() - 1)
        ...
        if file.read(1) == b"\x04":
            ...
            self.__password_validation_enc_data = file.read(80) # 64 + 16 (padding, needed by decrypt(), even though it's obsolete in this case)
        else:
            file.seek(file.tell() - 1)
        ...
        if self.__isencrypted:
            ...
            if self.__password_validation_enc_data is not None:
                decrypt(BytesIO(self.__password_validation_enc_data), pv_data := BytesIO(), self.__password, self.__aes_init_vector)
                pv_data.seek(0)

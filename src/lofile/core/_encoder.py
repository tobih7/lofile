...
from lofile.core.shared import BaseClass, DESCRIPTION_MAX_LENGTH, DataType, EncodeError, LogLvl, NONETYPE, TAG_MAX_LENGTH, TAG_VALID_CHARS, attrib_to_binary, compress, encrypt, timestamp_to_binary
...
class Encoder(BaseClass):
    ...
    def write(
        ...
        if raw is not None:
            if not isinstance(raw, (str, bytes, BufferedReader, NONETYPE)):
                raise TypeError(f"argument raw must be str, bytes or BufferedReader (open() in mode \"rb\"), not {type(raw).__name__}")
            ...
            if not isinstance(pyscript, (str, bytes, BufferedReader, NONETYPE)):
                raise TypeError(f"argument pyscript must be str, bytes or BufferedReader (open() in mode \"rb\"), not {type(pyscript).__name__}")
        ...
        if self.__compress:
            # decide whether to write to temporary file or already to the final file
            if self.__password is not None: # if this is the last step (so no encryption) directly write to the final file
    ...
    # ===  HEADER  === #
    def __flushheader(self, datatype: DataType): # datatype is known first when write() is called, so write() (which btw calls this method) passes the datatype
        ...
        # PASSWORD VALIDATION #
            ...
            if self.__password_validation:
                encrypt(BytesIO(pv_data := urandom(64)), pv_enc_data := BytesIO(), self.__password, self.__init_vector)
                pv_enc_data.seek(0)
                self.__file.write(b"\x04" + pv_data + pv_enc_data.read()) # 64 + 16 (padding, needed by decrypt(), even though it's obsolete in this case)
                del pv_data, pv_enc_data # no need to close BytesIO objects, __del__ should do that

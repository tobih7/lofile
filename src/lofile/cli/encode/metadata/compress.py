...
class Compress:
    ...
    def _set_compress(self):
        ...
        try:
            cmprs = Selector(("Yes", "No", None, "Yes, with custom compression level", None, "back"), print_result=False).pos
        ...
        if cmprs == 0:
            ...
            try:
                self.compression_level = Selector(("1", "2", "3", "4", "5", "6", "7", "8", "9"), "Compression Level:", print_result=False, start_pos=self.compression_level-1).pos + 1
            except KeyboardInterrupt:
                return
            self.compress = True

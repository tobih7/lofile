...
class Metadata(Data, Description, Tag, Timestamp, Password, Compress):
    ...
    def askmetadata(self):
        ...
        funcs = (self._set_data, self._set_description, self._set_tag, self._set_timestamp, self._set_password, self._set_compress)

        while True:
            out("\x1b[4;2H\x1b[J", flush=True) # using absolute pos, due to CSI s and CSI u being used inside the loop
            s = Selector(("\x1b[92mconfirm and proceed", None, *self.parsed_metadata()), print_result=False).pos
            if s == 0: return
            funcs[s-1]()
    ...
    def __parsed_description(self):
        if not self.description:
            return "\x1b[96mNo"
        return "\x1b[0m" + (dsc := self.description.splitlines())[0][:32].decode() + \

...
class Data:
        ...
        try:
            ...
            if self.datatype is DataType.Raw:
                if self.dataorigin[0] is DataOrigin.File:
                    ...
                    s = Selector(("change file", None, "change datatype", "back"), print_result=False).pos
                    if s == 0: self.data = self._data_raw(method=0) # 0 = askfile
                    elif s == 1: self.__reset_data()

                elif self.dataorigin[0] is DataOrigin.NotepadInput:
                    param("Origin", "Notepad Input")
                    end_header()

                    s = Selector(("edit data with notepad", None, "change datatype", "back"), print_result=False).pos
                    if s == 0:
                        self.__gototop()
                ...
                if self.dataorigin[0] is DataOrigin.File:
                    ...
                    s = Selector(("edit data with notepad   \x1b[90m(no effect on original file)", None, "change datatype", "back"), print_result=False).pos
                    if s == 0: self.__edit_json_data()
                    elif s == 1: self.__reset_data()

                if self.dataorigin[0] in (DataOrigin.NotepadInput, DataOrigin.ConsoleInput):
                    param("Origin", "Notepad Input" if self.dataorigin[0] == DataOrigin.NotepadInput else "Console Input")
                    end_header()
    ...
    def __edit_json_data(self):
        self.__gototop()
        self.data = self._data_json(editing=True, data=dumps(self.data, indent="\t").encode()) # editing means notepad input, no other possible input type

...
class Description:
    def _set_description(self):
        ...
        if self.description is None:
            ...
            out(
                ...
                *(
                    "  " + line
                    for line in self.description.decode().splitlines(keepends=True)
                ),
                "\n",
                vline(),
                "\n\n",
            ...
            try:
                s = Selector(
                    ("clear description", "new description", "back"), print_result=False
                ).pos
    ...
    def __ask_description(self):
        ...
        if Selector(
            ("notepad input", "console input"), "Data Source:", print_result=False
        ).pos:  # == 1
            out("\x1b[s")
            description = console_input(header=hint).encode("UTF-8")
            out("\x1b[u")
        else:  # == 0
            description = notepad_input(
                "description", header=hint + "\nAlso NUL-bytes are forbidden."
            ).read()

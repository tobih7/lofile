    ...
    try:
        with Encoder() as encoder:
            ...
            while True:
                out(
                    "\x1b[4;2H\x1b[J"
                )  # using absolute positions instead of relative ones (via \x1b[u) because \1b[s may be used during the loop and the saved position would be invalid

                o = Selector(
                    ("\x1b[92mconfirm and proceed", "", *parsed_options()),
                    print_result=False,
                    start_pos=o,
                ).pos
                out("\x1b[4H")
                ...
                if o == 0:
                    ...
                    try:
                        datatype = DataType[
                            Selector(
                                (i.name for i in DataType),
                                "Datatype:",
                                print_result=False,
                                can_terminate=False,
                                start_pos=datatype.value // 4,
                            ).result
                        ]
                    except KeyboardInterrupt:
                        continue
                    ...
                    def set_desc():
                        out("\x1b[s")
                        encoder.description = console_input(
                            header="The description can have a maximum length of 512 bytes."
                        ).encode("UTF-8")[:512]
                        out("\x1b[u")
                    ...
                    if encoder.description is None:
                        ...
                        try:
                            s = Selector(
                                ("clear description", "new description", "back"),
                                print_result=False,
                                can_terminate=False,
                            ).pos
                        except KeyboardInterrupt:
                    ...
                    try:
                        encoder.timestamp = not Selector(
                            ("Yes", "No"),
                            "Include creation date and time?",
                            print_result=False,
                            can_terminate=False,
                            start_pos=int(not encoder.timestamp),
                        ).pos
                    except KeyboardInterrupt:
                        continue
                    ...
                    if not encoder.is_password:
                        ...
                        try:
                            ...
                            if s == 0:
                                ...
                                out(
                                    vline(),
                                    "\r\x1b[4C\x1b[96m  Current Initialization Vector  \x1b[0m",
                                )
                                out(
                                    "\n\n  ",
                                ...
                                try:
                                    ...
                                    if iv_sel == 0:
                                        out(
                                            "\x1b[u  The initialization vector must have a length of 16.\n  Unicode characters will be encoded to UTF-8.\n\n"
                                        )
                                        while True:
                                            try:
                                ...
                                out(
                                    "  If password validation is enabled a hash will be stored in the file header.\n"
                                ...
                                try:
                                    encoder.password_validation = not Selector(
                                        ("Yes", "No"),
                    ...
                    try:
                        cmprs = Selector(
                            (
                                ...
                                "No",
                                "",
                                "Yes, with custom encryption level",
                                "",
                                "back",
                            ),
                            print_result=False,
...
def askpath_and_validate():
    ...
    while True:
        ...
        try:
            ...
            err(
                f"The path is invalid!\n\x1b[2C\x1b[90mError {exc.errno}: {exc.strerror}"
            )  # exc.strerror is same as os.strerror(exc.errno)
        else:

...
from loolclitools import Selector, askpath, out
...
class Data(Raw, JSON, Archive, PyScript):
    ...
    def askdata(self):
        datatype = DataType[Selector((i.name for i in DataType), "Datatype:",
                                     print_result=False, start_pos=self.datatype.value).result]

        self.data = (self._data_raw, self._data_json,
                     self._data_archive, self._data_pyscript)[datatype.value]()

    ...
    try:
        with Decoder(open(filepath, "br")) as decoder:
            ...
            if decoder.description is not None:
                description = decoder.description.replace(b"\r\n", b"\n").decode("UTF-8").replace("\n", f"\n\x1b[{LJUST + 2}C")
            ...
            if decoder.datatype == DataType.Raw:
                timeouts.start()
                s = Selector(("print to console", "save to file", "nothing"), "What to do with the data?", "->", False)
                timeouts.stop()
            ...
            # ===  JSON  === #
                ...
                if s != 2:
                    timeouts.start()
                    formatted = Selector(("Yes", "No"), "Format data? (indentation, etc.)", "->", False) == 0
                    timeouts.stop()
                    ...
                    # SAVE TO FILE #
                        ...
                        out("\r\x1b[K")
                        param("Data " + ("formatted and " if formatted else "") + "saved to", path)
            ...
            # ===  ARCHIVE  === #
                ...
                def extract_all():
                    ...
                    flush()
                    path = ospath.realpath(ospath.normpath(ospath.join(path, ospath.splitext(ospath.split(filepath)[1])[0])))
                ...
                def list_all_files():
                    ...
                    for i in sorted(elements.keys(), key=str.casefold):
                        out(f"\x1b[3C- {i}".ljust(max_path_length + 10), f"\x1b[90m{elements[i][0].rjust(max_num_length)} {elements[i][1]}\x1b[0m\n")
                    out("\n")


                def list_root_directory():
                    dirs = {} # contains each directory, each value is the number of subdirs and files inside the directory
                ...
                def enter_after_list_loop():
                    while True:
                        timeouts.start()
                        s = Selector(("extract all", "extract specific file", "list all files", "list root directory", "exit"), print_result=False)
                        timeouts.stop()
                        ...
                        # EXTRACT SPECIFIC FILE #
                            ...
                            final_filepath = ospath.realpath(ospath.normpath(ospath.join(path, ospath.split(selected_file.path)[1])))
                            ...
                            if ospath.exists(final_filepath):
                                ...
                                if ospath.isdir(final_filepath):
                                    ...
                                    out("\n")
                                    timeouts.start()
                                    overwrite = Selector(("Yes", "No"), "Overwrite?", print_result=False) == 0
                                    timeouts.stop()
                                    if overwrite:
                ...
                # START
                timeouts.start()
                s = Selector(("extract all", "list all files", "list root directory", "exit"), print_result=False)
                timeouts.stop()

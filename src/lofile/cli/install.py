...
def installer():
    ...
    INSTALLATION_SCRIPT = "&".join(
        [
            ...
            'del /f /q "%programfiles%\\lool\\lofile\\lofile{}"'.format(
                ".pyz" if os.path.splitext(TARGET_PATH)[1] == ".exe" else ".exe"
            ),
    ...
    if is_admin:
        ...
        errcode = windll.shell32.ShellExecuteW(
            None, "runas", os.getenv("COMSPEC"), "/c " + INSTALLATION_SCRIPT, None, 0
        )  # last param = nShowCmd
        if errcode == 5:
    ...
    if create_desktop_shortcut:
        call(
            [
                "powershell.exe",
                rf"$s=(New-Object -COM WScript.Shell).CreateShortcut('{os.getenv('USERPROFILE')}\Desktop\lofile CLI.lnk');$s.TargetPath='{os.getenv('PROGRAMFILES')}\lool\lofile\lofile{FILE_EXTENSION}';$s.IconLocation='{os.getenv('COMSPEC')},0';$s.Save()",
            ],
            stdout=DEVNULL,
            stderr=DEVNULL,
            stdin=DEVNULL,
